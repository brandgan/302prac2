...
public class ToDoList {
    ...
    public void onMarkAsDone() {
        ...
        if (itemNumber >= 0 && itemNumber < user.getToDoItems().size()) {
            ToDoItem itemToMark = user.getToDoItems().get(itemNumber);
            itemToMark.setDone(true);
    ...
    public void onMarkAsUndone() {
        ...
        if (itemNumber >= 0 && itemNumber < user.getToDoItems().size()) {
            ToDoItem itemToMark = user.getToDoItems().get(itemNumber);
            itemToMark.setDone(false);

            System.out.println("Item marked as undone successfully");
        }
        else {
            System.out.println("Invalid item number");
        }
